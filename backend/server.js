...
app.get('/', (req, res) => {
  res.json({ status: 'success', message: 'NOVA DESK IDE Backend is up and running!' });
});
  ...
  res.send(`
    ...
    <body>
      <div class="card">
        ...
        <h1>Sign in to NOVA DESK IDE</h1>
...
app.post('/api/chat', authenticateToken, async (req, res) => {
  ...
  const contents = nonSystemMsgs.map((m) => {
    ...
    // For function responses, the role must be 'user' according to Gemini API if we map it that way, or 'function'
...
app.listen(PORT, () => {
  console.log(`🚀 NOVA DESK IDE Backend running on http://localhost:${PORT}`);
